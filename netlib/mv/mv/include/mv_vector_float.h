...
{                                                                      
    ...
    MV_Vector_float();                             
    MV_Vector_float(unsigned int);                             
    MV_Vector_float(unsigned int, const float&);   // can't be inlined 
                                                     //because of 'for'
                                                    // statement.
    MV_Vector_float(float*, unsigned int);      // new copy
    MV_Vector_float(const float*, unsigned int);        // new copy ???
    
    // reference of an exisiting data structure
    //
    MV_Vector_float(float*, unsigned int, MV_Vector_::ref_type i);  
...
//    inline                operator const  float*() const {return p_;} 
    ...
    inline int                      null() const {return dim_== 0;}
            ...
            MV_Vector_float & newsize(unsigned int );
    ...
    friend ostream& operator<<(ostream &s, const MV_Vector_float &A);
