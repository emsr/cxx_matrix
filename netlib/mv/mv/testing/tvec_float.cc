...
main(int argc, char *argv[])
{
    ...
    cout << "Testing VECTOR_float &c = C(I)" << endl;
...
// result and hence not compilable on all C++ platforms.
    ...
    cout <<  " VECTOR_float E(&F(0),N) " << endl;

    VECTOR_float E(&F[0], N);
    cout << "E : " << endl << E << endl;
    ...
    cout <<  " VECTOR_float G(&F(0),N, MV_Vector_::ref) " << endl;

    VECTOR_float G(&F[0], N, MV_Vector_::ref);
    ...
    cout << "Testing returning subvectors from functions:  " 
        << endl << "   VECTOR_float K = fexample(A,1.0); " << endl;
    //
    VECTOR_float K = fexample(A,1);
    //
    cout << "A" << endl << A << endl;
    cout << "K" << endl << K << endl;


    VECTOR_float Q( &A(0), A.size(), MV_Vector_::ref);
    A() = B;
    //
