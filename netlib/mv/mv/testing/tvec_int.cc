...
main(int argc, char *argv[])
{
    ...
    cout << "Testing VECTOR_int &c = C(I)" << endl;
...
// result and hence not compilable on all C++ platforms.
    ...
    cout <<  " VECTOR_int G(&F(0),N, MV_Vector_::ref) " << endl;

    VECTOR_int G(&F[0], N, MV_Vector_::ref);
    ...
    cout << "Testing returning subvectors from functions:  " 
        << endl << "   VECTOR_int K = fexample(A,1.0); " << endl;
    //
    VECTOR_int K = fexample(A,1);
    //
    cout << "A" << endl << A << endl;
    cout << "K" << endl << K << endl;


    VECTOR_int Q( &A(0), A.size(), MV_Vector_::ref);
    A() = B;
    //
