...
template<typename _NumTp, typename _Matrix>
  ...
  public:
    ...
    using value_type = decltype(_Matrix{}[0][0]);
...
/**
 *  Constructs the QR decomposition of a[0..n_rows - 1][0..n_cols - 1].  The upper triangular _Matrix R
 *  is returned in the upper triangle of a except the diagonal elements of R which are returned
 *  in d[0..n_cols - 1].  The orthogonal _Matrix Q is represented as a product of n - 1 Householder
...
template<typename _Matrix, typename _Vector>
            ...
            _Vector& c, _Vector& d, bool& singular)
  {
    ...
    for (std::size_t k = 0; k < n_cols - 1; ++k)
      {
	//  See if the _Matrix is singular in this column.
 ...
 *  The inputs are the QR decomposition of the _Matrix in a[0..n_rows - 1][0..n_cols - 1],
...
template<typename _Matrix, typename _Vector>
  ...
  qr_backsub(const std::size_t n_rows, const std::size_t n_cols,
             const _Matrix& a,
             const _Vector& c, const _Vector& d,
             _Vector& b)
  {
    using _NumTp = std::remove_reference_t<decltype(a[0][0])>;
 ...
 *  _Matrix stored in a[0..n_rows - 1][0..n_cols - 1] and d[0..n_cols - 1].
 *  Here n_rows >= n_cols.
 */
template<typename _Matrix, typename _Vector>
  void
...
/**
 *  Inverts a _Matrix given the QR decomposed _Matrix.
 *  The inverse _Matrix is allocated in this routine so make sure the pointer is freed first.
 *
 *  The inverse _Matrix is NOT in QR form.
 */
template<typename _Matrix, typename _Vector>
  void
  qr_invert(std::size_t n_rows, std::size_t n_cols,
            ...
            _Matrix& a_inv)
  {
    using _NumTp = decltype(a_qr[0][0]);
...
template<typename _Matrix, typename _Vector>
            ...
            _Vector& u, _Vector& v)
  {
    ...
    //  Transform upper Hessenberg _Matrix to upper triangular.
